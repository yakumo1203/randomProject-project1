...
router.get('/home/:university_id/:user_id', (req, res) => {
  const university_id = req.param.university_id;
  const user_id = req.param.user_id;
  const queryText = 
    `SELECT * FROM universities LEFT JOIN university_application_info
    ON universities.id = university_application_info.university.id
    WHERE id = ?;`;
  conn.query(queryText, [university_id], (err, result) => {
    if (err) throw err;
    res.json(result);
  });
});
